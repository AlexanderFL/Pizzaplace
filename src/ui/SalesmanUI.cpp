...
SalesmanUI::makeYourOwnMenu(Order &order, string &input, const bool &isFromMenu)
{
    ...
    while (true)
    {
        ...
        switch (selection)
        {
            case 1: selectCrust(order, input);
                printMessage("Crust added.");
                break;
            case 2: selectSize(order, input);
                printMessage("Size added.");
                break;
            case 3: selectToppings(order, input);
                printMessage("Toppings added.");
                break;
            case 4: selectSides(order, input);
                printMessage("Side added.");
                break;
            case 5: selectDeliveryMethod(order, input);
                printMessage("Delivery method added.");
                break;
            case 6: addComment(order);
                printMessage("Comment added");
                break;
            case 7: addAnotherPizza(order);
                printMessage("You are now making a new pizza");
                break;
            case 8: showTotalOrder(order, input);
                break;
            case 9: markOrderAsPaid(order);
                printMessage("Order has been marked as paid.");
                break;
            case 10: finishOrder(order);
                return;
            case 11: clear();
                return;
            default: return;
...
SalesmanUI::showSinglePizza(Order &order, const int &index, string &input)
{
    while (true)
    {
        ...
        if (inputInt != 6)
        {
            switch (inputInt)
            {
                case 1: showToppings(pizzaThatIsBeingEdited, input);
                    break;
                case 2: showCrust(pizzaThatIsBeingEdited, input);
                    break;
                case 3: showSize(pizzaThatIsBeingEdited, input);
                    break;
                case 4: selectPizza(index);
                    printMessage("You are now editing pizza nr. " + to_string(index + 1));
                    return;
                case 5: deletePizza(order, index);
                    return;
            }
        }
        else
